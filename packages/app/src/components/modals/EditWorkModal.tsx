...
export default function EditWorkModal() {
	...
	useEffect(() => {
		if (!modal.isOpen) {
			...
			const date = new Date(modal.work.timeOfEntry);
			setDate(`${date.getFullYear()}-${(date.getMonth() + 1).toString().padStart(2, "0")}-${date.getDate().toString().padStart(2, "0")}`);

			const entryTime = new Date(modal.work.timeOfEntry);
			setEntryHour(hourOptions.find((x) => x.value === entryTime.getHours()) ?? hourOptions[0]);
			setEntryMinute(getClosestMinuteOption(entryTime.getMinutes()));

			if (modal.work.timeOfExit) {
	...
	function getClosestMinuteOption(minute: number) {
		const closestMinute = minuteOptions.reduce((prev, curr) => (Math.abs(curr.value - minute) < Math.abs(prev.value - minute) ? curr : prev));
	...
	function close() {
		updateModals({ editWork: { isOpen: false } });
	}
	...
	async function edit() {
		if (updateWorkMutation.isPending || !date || !modal.work) {
			return;
		}
